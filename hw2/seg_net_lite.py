...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        for i in range(self.num_up_layers):
            ...
            layers_conv_up.append(
                nn.Conv2d(
                    ...
                    padding=conv_paddings[i]))
            layers_bn_up.append(
                nn.BatchNorm2d(num_features=up_filter_sizes[i]))
        ...
        self.final_layer = nn.Conv2d(in_channels=input_size, out_channels=11, kernel_size=1)

    def forward(self, x):
        indices_list = []
        for i in range(self.num_down_layers):
            # each block should consist of: Conv2d->BatchNorm2d->ReLU->MaxPool2d
            x, indices = self.layers_pooling[i](self.relu(self.layers_bn_down[i](self.layers_conv_down[i](x))))
            indices_list.insert(0, indices)

        for i in range(self.num_up_layers):
            # each block should consist of: MaxUnpool2d->Conv2d->BatchNorm2d->ReLU
            x = self.relu(self.layers_bn_up[i](self.layers_conv_up[i](self.layers_unpooling[i](x, indices_list[i]))))
